...
//	double H = 0.1; // Thickness of growing layer
	double* H = new double[nn]; // Thickness of growing layer
...
//	const double alfagt = 1.9;// Tangential growth rate of gray matter
	double* alfagt = new double[nn];
	for (int i = 0; i < nn; i++) {	
		Vector qp = Ut0[i];
		double rqp = Vector(qp.x, 0, qp.z).length(); // for a small circle
		alfagt[i] = 1.9;
//		if (qp.y <-0.7 && rqp < 0.25){
//			alfagt[i] = 1*alfagt[i]; // modify the growth rate locally
//		}
	}
	
	
	const double alfagn = 0;// Growth rate of gray matter in thickness direction  log(1.0)
	const double alfaw = 0;// Isotropic growth rate of white matter  log(1.0)
	const double alfar = 0;// Rate of stress relaxation 0.0
	...
	//vector<int> NNLt[nsn]; // Triangle-proximity lists for surface nodes
...
//		double rqp = Vector(0, qp.y, qp.z).length(); 
	...
	while (t < 1.0) { // Main loop
		...
		for (int i = 0; i < nn; i++) {	
			Vector qp = Ut0[i];
			double rqp = (qp.x/sqrt(2)+qp.z/sqrt(2))*(qp.x/sqrt(2)+qp.z/sqrt(2))/(0.5*0.5) + (qp.x/sqrt(2)-qp.z/sqrt(2))*(qp.x/sqrt(2)-qp.z/sqrt(2))/(0.3*0.3) ; // for a small circle
			H[i] = 0.1 - 0.05*t; 
			if (qp.y <-0.6 && rqp < 1){  
		...
		for (int i = 0; i < ne; i++) {
			...
			double gm = 1.0/(1.0 + exp(10.0*(0.25*(d2s[n1]+d2s[n2]+d2s[n3]+d2s[n4])/(0.25*(H[n1]+H[n2]+H[n3]+H[n4])) - 1.0)))* 0.25*(gr[n1]+gr[n2]+gr[n3]+gr[n4]); 
			double vm = 1.0 - gm;
			...
			// tangential 
			Matrix Lggt = (I - Matrix(Ns.x*Ns.x, Ns.x*Ns.y, Ns.x*Ns.z, Ns.x*Ns.y, Ns.y*Ns.y, Ns.y*Ns.z, Ns.x*Ns.z, Ns.y*Ns.z, Ns.z*Ns.z))*(0.25*(alfagt[n1]+alfagt[n2]+alfagt[n3]+alfagt[n4]));
			// normal
...
void createNNLtriangle(vector<int>* NNLt, Vector* Ut, vector<Face>& faces, int* SN, int nsn, int nf, double hs, double mw) {
	...
	for (int i = 0; i < nf; i++) { // Divide triangles into cells
		...
		if(mx*mx*za + mx*ya + xa < mx*mx*mx && mx*mx*za + mx*ya + xa >=0){
