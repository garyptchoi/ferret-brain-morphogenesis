// Modifying the growth rate (0.5x) in the brain growth simulation
#include <omp.h>
#include "vema.h"
#include "eig3.h"
#include <stdlib.h>
...
int main(int argc, char* argv[]) {
	...
	const double alfagt = 1.9*0.5;// Tangential growth rate of gray matter
...
//		double rqp = Vector(0, qp.y, qp.z).length(); 
	...
	while (t < 1.0) { // Main loop
		...
		for (int i = 0; i < ne; i++) {
			...
			Matrix Lgg = Lggt + Lggn; // the second term is 0
			Matrix Lgw = I*alfaw; // 0
			Matrix Lg = Lgg*gm + Lgw*vm; //  = Matrix(...)*alfagn*gm
